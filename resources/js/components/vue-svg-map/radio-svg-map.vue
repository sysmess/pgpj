...
export default {
  ...
  computed: mapGetters(['Status', 'Classes', 'ClassesCount', 'SelectedRegionDataFiltered', 'RussiaDataFiltered']),
  ...
  created() {
    this.unsubscribe = this.$store.subscribe((mutation, state) => {
      if (mutation.type === 'SelectRegion') {
        //console.log(`Updating to ${state.SelectedRegion.name}`);

        // Do whatever makes sense now
        this.$emit("change", state.SelectedRegion.id.toString());
      }
    });
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    selectLocation(location) {
      ...
      if (location.id !== this.value) {
        ...
        this.$store.dispatch("SelectRegion", location.id);
    ...
    selectNextLocation(event) {
      ...
      const nextLocation =
        nextSibling && nextSibling.nodeName === "path"
          ? nextSibling
          : this.locations[0];
